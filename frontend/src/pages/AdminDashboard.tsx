...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const pendingResourcesData = await authFetch('http://localhost:4000/api/resources/admin/pending');
  ...
  const handleApproveResource = async (resourceId: number) => {
    try {
      await authFetch(`http://localhost:4000/api/resources/${resourceId}/approve`, {
        method: 'PUT'
      });
      ...
      setPendingResources(pendingResources.filter(resource => resource.id !== resourceId));
    } catch (error) {
  ...
  const handleRejectResource = async (resourceId: number) => {
    try {
      await authFetch(`http://localhost:4000/api/resources/${resourceId}/reject`, {
        method: 'PUT'
      });
  ...
  return (
    <Box minH="calc(100vh - 70px)" bg="gray.50" p={8}>
      <Container maxW="1200px">
        <Heading as="h1" mb={8} size="xl" color="blue.800">Admin Dashboard</Heading>
        
        <Tabs variant="enclosed" colorScheme="blue" bg="white" borderRadius="lg" boxShadow="md" overflow="hidden">
              ...
              {loading ? (
                ...
                <Box overflowX="auto">
                  ...
                  >
                    <Thead bg="blue.700">
                      <Tr>
                        <Th color="white" fontWeight="bold">Title</Th>
                        <Th color="white" fontWeight="bold">Type</Th>
                        <Th color="white" fontWeight="bold">Subject</Th>
                        <Th color="white" fontWeight="bold">Submitted By</Th>
                        <Th color="white" fontWeight="bold">Actions</Th>
                      </Tr>
                    </Thead>
                    <Tbody>
                      {pendingResources.map((resource, idx) => (
                        ...
                        >
                          <Td fontWeight="medium" color="gray.900">{resource.title}</Td>
                          <Td color="gray.900">{resource.type}</Td>
                          <Td color="gray.900">{resource.subject}</Td>
                          <Td color="gray.900">{resource.user?.firstName} {resource.user?.lastName}</Td>
                          <Td>
                            <Button 
                              size="sm" 
                              bgColor="green.600" 
                  ...
                  >
                    <Thead bg="blue.700">
                      <Tr>
                        ...
                        <Th color="white" fontWeight="bold">Email</Th>
                        <Th color="white" fontWeight="bold">Role</Th>
                        <Th color="white" fontWeight="bold">Status</Th>
                        <Th color="white" fontWeight="bold">Actions</Th>
                      </Tr>
                    </Thead>
                    <Tbody>
